...
addEventHandler( "onResourceStart", root, function ( res )
	...
	for _, mrkr in ipairs( getElementsByType'randommarker', res ) do
...
-- fun below
local vehs={[0]={[0]={[255]={602,545,496,517,401,410,518,600,527,436,589,580,419,439,533,549,526,491,474,445,467,604,426,507,547,585,405,587,409,466,550,492,566,546,540,551,421,516,529,581,510,509,522,481,461,462,448,521,468,463,586,485,552,431,438,437,574,420,525,408,416,596,433,597,427,599,490,528,601,407,428,544,523,470,598,499,588,609,403,498,514,524,423,532,414,578,443,486,406,531,573,456,455,459,543,422,583,482,478,605,554,530,418,572,582,413,440,536,575,534,567,535,576,412,402,542,603,475,568,557,424,471,504,495,457,539,483,508,571,500,411,515,444,556,429,541,559,415,561,480,560,562,506,565,451,434,558,494,555,502,477,503,579,400,404,489,505,479,442,458,441,464,594,501,465,564,432,},},[255]={[0]={472,473,493,595,484,430,453,452,446,454,},},},[255]={[0]={[0]={592,577,511,548,512,593,425,520,417,487,553,488,497,563,476,447,519,460,469,513,},},},}
local col={vehicle={0,0,255},boat={0,255,0},air={255,0,0}}
local g_Checkpoints
function ntscheckpoints(checkpoints)
	for k,c in ipairs(checkpoints) do
		-- if not(vehs[c.color[1]] and vehs[c.color[1]][c.color[2]] and vehs[c.color[1]][c.color[2]][c.color[3]]) then
			-- outputChatBox("WARNING: Checkpoint \"" .. c.id .. '" doesn\'t have a valid NTS colour', root, 255,0,0)
		-- end
...
function ntsCheckpoint ( i )
	...
	local color = col[tostring(getElementData(ch, 'nts') )] or {}
	setElementModel(getPedOccupiedVehicle(source), vehs[color[1]][color[2]][color[3]] and vehs[color[1]][color[2]][color[3]][math.random(#vehs[color[1]][color[2]][color[3]])] or getElementModel(getPedOccupiedVehicle(source)))
end

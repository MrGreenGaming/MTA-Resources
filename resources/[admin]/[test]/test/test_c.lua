...
setTimer(function()
	...
	for k,p in ipairs(getElementsByType'player') do
		if getElementData(p, 'player state') == 'alive' then
			local veh = getPedOccupiedVehicle(p)
			if not veh then
				outputDebugString('No vehicle found: ' .. getPlayerName(p) .. ' try spectating to fix')
				outputConsole('No vehicle found: ' .. getPlayerName(p) .. ' try spectating to fix')
			elseif getElementDimension(p) ~= getElementDimension(veh) then
				outputDebugString('Vehicle dimension found: ' .. getPlayerName(p) .. ' try spectating to fix')
				outputConsole('Vehicle dimension found: ' .. getPlayerName(p) .. ' try spectating to fix')
			end
		end
	end
end, 1000, 0)

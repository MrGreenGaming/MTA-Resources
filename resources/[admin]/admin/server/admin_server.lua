...
function aSetPlayerMuted ( player, state, length, pAdmin)
...
addEventHandler ( "onPlayerJoin", _root, function ()
	...
	for id, player in ipairs(getElementsByType("player")) do
		if ( hasObjectPermissionTo ( player, "general.adminpanel" ) ) then
			triggerClientEvent ( player, "aClientPlayerJoin", source, getPlayerIP ( source ), getPlayerUserName ( source ), getPlayerAccountName ( source ), getPlayerSerial ( source ), hasObjectPermissionTo ( source, "general.adminpanel" ), aPlayers[source]["country"], exports.gc:getPlayerForumID ( source ), exports.gc:getPlayerGreencoinsLogin ( source ) )
		end
	end
	...
	if ( hasObjectPermissionTo ( source, "command."..action ) ) then
		...
		if ( action == "kick" ) then
            ...
            if getResourceFromName('discord') and getResourceState(getResourceFromName('discord')) == 'running' then
                exports.discord:send("admin.log", { log = getAccountName( getPlayerAccount(source)) .. " kicked " .. remcol(getPlayerName(player)) .. " (" .. reason .. ")"})
            end
		elseif ( action == "ban" ) then
			...
			if bUseSerial then
                ...
                if getResourceFromName('discord') and getResourceState(getResourceFromName('discord')) == 'running' then
                    exports.discord:send("admin.log", { log = getAccountName( getPlayerAccount(source)) .. " banned " .. getPlayerSerial(player) .. " (" .. reason .. ")"})
                end
		...
		elseif ( action == "sethealth" ) then
			...
			if ( health ) then
				if ( health > 1000 ) then health = 500 end
				if (health < 250) then health = 300 end
				if ( not setElementHealth ( getPedOccupiedVehicle(player), health ) ) then
					action = nil
				end
				mdata = health
			else
		...
		elseif ( action == "slap" ) then
			if ( getElementHealth ( getPedOccupiedVehicle(player) ) > 0 ) and ( not isPedDead ( player ) ) then
				if ( ( not data ) or ( not tonumber ( data ) ) ) then data = 20 end
				if ( ( tonumber ( data ) >= 0 ) ) then
					if ( tonumber ( data ) > getElementHealth ( getPedOccupiedVehicle(player) ) ) then setTimer ( killPed, 50, 1, player )
					else setElementHealth ( getPedOccupiedVehicle(player), getElementHealth ( getPedOccupiedVehicle(player) ) - data ) end
					local x, y, z = getElementVelocity ( getPedOccupiedVehicle(player) )
					setElementVelocity ( getPedOccupiedVehicle(player), x , y, z + 0.2 )
					mdata = data

                    if getResourceFromName('discord') and getResourceState(getResourceFromName('discord')) == 'running' then
                        exports.discord:send("admin.log", { log = getAccountName( getPlayerAccount(source)) .. " slapped " .. remcol(getPlayerName(player))})
                    end
				else
					action = nil
				end
...
addEventHandler ( "aMessage", _root, function ( action, data )
	...
	if ( action == "new" ) then
		...
		if getResourceFromName('irc') and getResourceState(getResourceFromName('irc')) == "running"  then
			if exports.irc then
				local message = "Report was submitted by "..getPlayerName(source)..". Subject: " .. tostring(data.subject) .. ". Message: "..tostring(data.message)
...
addEventHandler ( "aBans", _root, function ( action, data )
	...
	if ( hasObjectPermissionTo ( source, "command."..action ) ) then
		...
		if ( action == "banip" ) then
			mdata = data
            if getResourceFromName('discord') and getResourceState(getResourceFromName('discord')) == 'running' then
                exports.discord:send("admin.log", { log = getAccountName( getPlayerAccount(source)) .. " banned an IP"})
            end
		...
		elseif ( action == "banserial" ) then
			...
			if ( isValidSerial ( data ) ) then
                if getResourceFromName('discord') and getResourceState(getResourceFromName('discord')) == 'running' then
                    exports.discord:send("admin.log", { log = getAccountName( getPlayerAccount(source)) .. " banned serial " .. string.upper(data)})
                end
		...
		elseif ( action == "unbanip" ) then
			...
			action = nil
            if getResourceFromName('discord') and getResourceState(getResourceFromName('discord')) == 'running' then
                exports.discord:send("admin.log", { log = getAccountName( getPlayerAccount(source)) .. " unbanned an IP"})
            end
		...
		elseif ( action == "unbanserial" ) then
			...
			action = nil
            if getResourceFromName('discord') and getResourceState(getResourceFromName('discord')) == 'running' then
                exports.discord:send("admin.log", { log = getAccountName( getPlayerAccount(source)) .. " unbanned serial " .. string.upper(data)})
            end
			for i,ban in ipairs(getBans ()) do
...
function addMuteToDB(serial, pName, length, adminAccountName, pIP)
	if serial and #serial == 32 and length and adminAccountName then
		local query = dbExec(dbHandler,"REPLACE INTO mute (serial, name, expireTimestamp, byAdmin, IP) VALUES (?,?,?,?,?)", serial, pName or "", getExpireTimestamp(length), adminAccountName, pIP or "")
			if query then
				outputServerLog( "* Mutes DB:".. (pName or serial).. "" ..(ifNameReturnSerial() or "").. " has been muted for " ..secondsToTimeDesc(length).. " by " ..adminAccountName )
			else
				outputChatBox("Failed to add mute to DB. Player: "..pName.. ", length: "..secondsToTimeDesc(length)..", by admin: "..adminAccountName, root, 255, 0, 0)
				outputServerLog("Failed to add mute to DB. Player: "..pName.. ", length: "..secondsToTimeDesc(length)..", by admin: "..adminAccountName)
			end
		return query
	end
end
...
function serialmute(pAdmin, _, serial, days)
...
function serialunmute(pAdmin, _, serial)
	if not hasObjectPermissionTo ( pAdmin, "command.ban" ) then
		return
	end
    ...
    if getResourceFromName('discord') and getResourceState(getResourceFromName('discord')) == 'running' then
        exports.discord:send("admin.log", { log = "Serial: " ..serial.. " has been unmuted by " ..getAccountName( getPlayerAccount(pAdmin))})
    end
end

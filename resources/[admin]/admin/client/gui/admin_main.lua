...
y=y+B  aTab1.VehicleHealth	= guiCreateLabel ( 0.25, y, 0.25, 0.04, "Vehicle Health: 0%", true, aTab1.Tab )
		...
		aTab2.ResourceSettings	= guiCreateButton ( 0.40, 0.05, 0.20, 0.04, "Settings", true, aTab2.Tab, "resourcesettings" )
...
function aClientDoubleClick ( button )
	...
	elseif ( source == aTab2.ResourceList ) then
		if ( guiGridListGetSelectedItem ( aTab2.ResourceList ) ~= -1 ) and guiGetEnabled(aTab2.ResourceSettings) then
...
function loadFlagImage( guiStaticImage, countryCode )
	if countryCode then
		local flagFilename = "client\\images\\flags_new\\"..tostring ( countryCode )..".png"

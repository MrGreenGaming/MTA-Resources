...
function resetBurnUpTime(player, times, marker, marker1)
	...
	exports.messages:outputGameMessage(_.For(player, 'You burned a little longer! ') .. (times or ''), player)
end
...
function onPlayersVehicleCollide (player1, car1, player2, car2)
	...
	if g_PlayersBurnTransfer[player1] and isPerkAllowedInMode(ID) and getElementHealth(car1) <= 245 and getElementHealth(car2) > 245 and not isVehicleBlown(car1) and not isVehicleBlown(car2) then  --isOnFire?
		...
		outputChatBox(_.For(player1, "You have taken %s's health!"):format(getPlayerName(player2)), player1, 50, 202, 50)
		exports.messages:outputGameMessage(_.For(player1, "You have taken %s's health!"):format(getPlayerName(player2)), player1, 2, 50, 202, 50, true)

		outputChatBox(_.For(player2, '%s has given you his vehicle fire!'):format(getPlayerName(player1)), player2, 50, 202, 50)
		exports.messages:outputGameMessage(_.For(player2, '%s has given you his vehicle fire!'):format(getPlayerName(player1)), player2,2, 50, 202, 50, true)
	...
	elseif g_PlayersBurnTransfer[player2] and isPerkAllowedInMode(ID) and getElementHealth(car2) <= 245 and getElementHealth(car1) > 245 and not isVehicleBlown(car1) and not isVehicleBlown(car2) then   --isOnFire
		...
		outputChatBox(_.For(player2, "You have taken %s's health!"):format(getPlayerName(player1)), player2, 50, 202, 50)
		exports.messages:outputGameMessage(_.For(player2, "You have taken %s's health!"):format(getPlayerName(player1)), player2, 2, 50, 202, 50, true)

		outputChatBox(_.For(player1, '%s has given you his vehicle fire! '):format(getPlayerName(player2)), player1, 50, 202, 50)
		exports.messages:outputGameMessage(_.For(player1, '%s has given you his vehicle fire! '):format(getPlayerName(player2)), player1,2, 50, 202, 50, true)
	end
end

...
function receiveImage ( image, pid )
	...
	if image == false then
		...
		if fileGetSize(dummy) > (maxFileSize*1048576) then -- If file is bigger then x MB
			outputChatBox(_.For(player, "Custom paintjob image too big, please use a smaller image. (Maximum: ${size} MB)") % {size=tostring(maxFileSize)},player,255,0,0)
			fileClose(dummy)
...
function receiveHostingFetch(responseData,errno,player,id)
	...
	if errno == 0 then
		...
		outputChatBox(_.For(player, "Downloading paintjob failed, please check the URL or contact staff (ERROR: ${error} )") % {error=tostring(errno)},player,255,0,0)
	end
end

...
function gcshopRequestPaintjob(imageMD5, forumID, filename, id)
	...
	if string.find(filename,".tinypic.com/") or string.find(filename,"i.imgur.com/") or string.find(filename,".postimg.org/") then -- If filename is hosting URL
		...
		if forbiddenFile ~= 1 then -- If file has forbidden extension
			outputChatBox(_('Please use a different file extension. Only ${extensions} files are allowed') % {extensions=table.concat(allowedExtensions, ", ")}, 255, 0, 0)
		...
		if not fileExists(filename) then
			...
			outputChatBox(_('Custom paintjob image ${file} not found!') % {file=tostring(filename)}, 255, 0, 0)
		else
			...
			elseif not isValidImage(filename) then
				outputChatBox(_('Custom paintjob image ${file} is not a valid image') % {file=tostring(filename)}, 255, 0, 0)
			else

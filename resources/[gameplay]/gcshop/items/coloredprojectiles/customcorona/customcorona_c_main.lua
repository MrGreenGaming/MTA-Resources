...
funcTable = {}
local img = dxCreateTexture( "items/coloredprojectiles/customcorona/textures/coronastar.png", "dxt5" )
...
function funcTable.createShader(cType,nrType)
	local pathString = "items/coloredprojectiles/customcorona/shaders/custom_corona"..(cType-1)..".fx"	
	local theShader = nil
	if nrType then
		if (nrType == 1) then 
...
addEventHandler("onClientResourceStart", getResourceRootElement( getThisResource()), function()
	if not isMTAUpToDate() then 
		outputDebugString('Custom Coronas: The resource is not compatible with this client version!',255,0,0)
		return
	end
	...
	if not img then
		outputDebugString('Custom Coronas: Could not load the corona texture!',255,0,0)
		return
	end

...
--  [id]= { ID,     price,         description,               , loading function,     				, expire length in days,		allowed modes filter},
	[1] = { ID = 1, price =  3000, description = _.register('Bike trials'), func = 'loadGCTrials'},
	[2] = { ID = 2, price =  3250, description = _.register('Faster respawn for 60 days'), func = 'loadGCRespawn', exp = 60},
	[3] = { ID = 3, price =  2250, description = _.register('HP Regen for 30 days'), func = 'loadGCRegenHP', exp = 30},
	[4] = { ID = 4, price = 15000, description = _.register('Custom paintjob'), func = 'loadGCCustomPaintjob', defaultAmount = 1, extraPrice = 15000, modes = {'Sprint', 'Never the same', 'Manhunt'}},
	[5] = { ID = 5, price =  1500, description = _.register('Voice'), func = 'loadGCVoice'},
	[6] = { ID = 6, price =  2250, description = _.register('Longer burnup time'), func = 'loadGCBurn', modes = {'Sprint', 'Never the same', 'Shooter', 'Deadline', 'Reach the flag', 'Capture the flag', 'Destruction derby'}},
	[7] = { ID = 7, price =  3250, description = _.register('Extra long burnup time'), func = 'loadGCBurnExtra', requires = {6}, exp = 30, modes = {'Sprint', 'Never the same', 'Shooter', 'Deadline', 'Reach the flag', 'Capture the flag', 'Destruction derby'}},
	[8] = { ID = 8, price =  3000, description = _.register('Health transfer'), func = 'loadGCBurnTransfer', requires = {6}, exp = 30, disabled = true},
	[9] = { ID = 9, price =  3000, description = _.register('Double sided objects'), func = 'loadDoubleSided'},
	[10] = { ID = 10, price =  500, description = _.register('Colored Projectiles for 30 days'), func = 'loadProjectileColor', exp = 30},
	[11] = { ID = 11, price =  1500, description = _.register('NTS/DD Vehicle reroll for 30 days'), func = 'loadVehicleReroll', exp = 30},
	[12] = { ID = 12, price =  500, description = _.register('Deadline color change'), func = 'loadDeadlineColorChange', exp = 30},
		...
		function(expired)
			for _, perk in pairs(prks) do
				if tonumber(perk.exp) and (not expired[perk.ID] or expired[perk.ID] < getTimestamp()) then
					...
					outputChatBox ( _.For(theSource, 'GC: ${perk} has expired!') % {perk=_.For(theSource, perk.description)}, theSource, 255, 0, 0)
...
function buyPerk ( player, cmd, ID )
	...
	elseif not ID or type(i) ~= 'table' then
		outputChatBox(_.For(player, 'Not a valid perk (use the correct name or ID)'), player, 255, 0, 0 )
		return
	elseif i.disabled then
		outputChatBox(_.For(player, 'Perk purchase has been disabled'), player, 255, 0, 0 )
		return
	end
	...
	function(res)
		if res and res[ID] then
			outputChatBox(_.For(player, 'You already bought this perk: ${perk}') % {perk=_.For(player, i.description)}, player, 255, 165, 0 )
			return
		elseif i.requires and res then
			for _, perk in ipairs(i.requires) do
				if not res[perk] then
					outputChatBox(_.For(player, 'This perk requires another perk: ${perk}') % {perk=_.For(player, perks[perk].description)}, player, 255, 0, 0 )
					return
				end
			end
		end
		...
		if result == true then
			...
			outputChatBox (_.For(player, 'Perk "${perk}" bought.') % {perk=_.For(player, i.description)}, player, 0, 255, 0)
			loadPerk( player, ID )
			getPerks(forumID,
			function(thePerks)
		...
		if error then
			outputChatBox( _.For(player, 'An error occured, try reconnecting and notify an admin if you lost GC!'), player, 255, 0, 0 )
...
function buyPerkExtra ( player, cmd, ID, total )
	...
	elseif not total then
		outputChatBox(_.For(player, 'Not a valid amount (use a number)'), player, 255, 0, 0 )
		return
	end
	...
	if result == true then
		...
		function(perkSetting)
			...
			function(added)
				...
				outputChatBox (_.For(player, 'Extra perk "${perk}" bought.') % {perk=_.For(player, i.description)}, player, 0, 255, 0)
				--loadPerk( player, ID )
...
function flipPlayer(player)
	...
	if gcshopBuyItem ( player, items.Flip.price, 'Flip' ) then
		...
		outputChatBox ( _.For(player, 'You got flipped!'), player, 0x00, 0xFF )
	end
end
...
function callSpawn ( player )
	...
	elseif not spawnIndex[player] then
		if gcshopBuyItem ( player, items.Spawn.price, 'Spawn' ) then
			...
			outputChatBox(_.For(player, '[GC] Use shift+3 to go back to a previous spawnpoint'), player, 0,255,0)
		end
...
function setPlayerRocketColor(player,color)
	...
	if not color or #color ~= 7 then outputChatBox(_.For(player, "Something went wrong, please try again!"),player) return false end
	if not fID then outputChatBox(_.For(player, "You are not logged in!"),player) return false end

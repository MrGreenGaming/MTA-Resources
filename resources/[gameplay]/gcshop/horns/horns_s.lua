				...
				local query = dbQuery(
					function(qh)
						...
						if #sql > 0 then
							...
							if not useHorn then outputChatBox(_.For(player, "Please buy the horn (${hornName}) before using it") % {hornName=tostring(hornID or arg2)},player,255,0,0) return end

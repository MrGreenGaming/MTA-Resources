...
function(choice)
    ...
    if isPlayerEligibleToBuy(source, choice) then
        if playerHasBoughtMap(source, choice) then
            ...
            outputChatBox("[Maps-Center] ".. _.For(source, "You do not have enough GCs to buy a nextmap. Current price: ${price}") % {price=tostring(mapprice)}, source, 255, 0, 0)
        end
    else
...
function isDailyLimitReached(mapname)
    ...
    if theAmountTable[mapname] then
        ...
        if ( getRealTime().timestamp-theAmountTable[mapname].timestamp ) < mp_cooldownTime then
            ...
            outputChatBox("[Maps-Center] ".. _.For(source, "This map has recently been bought and is still on cooldown (${remain} remaining), please choose another map or wait.") % {remain=tostring(timeLeft)}, source, 255, 0, 0)
...
function queue(choice, player)
    ...
    outputChatBox("[Maps-Center] ".. _.For(player, "Your next map current queue number: ${num}") % {num=#myQueue}, player, 0, 255, 0)

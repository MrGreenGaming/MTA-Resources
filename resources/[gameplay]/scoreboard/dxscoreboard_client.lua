					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						if content then
							...
							elseif type( content ) == "table" and column.name ~= "name" then
								...
								dxDrawText( content, topX+theX, 		y, 		topX+x+s(column.width), 	y+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 			tocolor( r or 255, g or 255, b or 255, a or 255 ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI, true )
							end
						end
						x = x + s(column.width + 10)
					end
...
addEvent("controllerShowScoreboard")
addEventHandler("controllerShowScoreboard", root, function()
    executeCommandHandler("Toggle scoreboard", 1)
end)
addEvent("controllerHideScoreboard")
addEventHandler("controllerHideScoreboard", root, function()

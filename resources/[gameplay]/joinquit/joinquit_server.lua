...
function redirect(source, _ , playerName, ip, port)
    if not playerName or not ip then
        outputChatBox('Syntax: /redirect [player] [ip] [port]', source, 255, 100, 100)
        return
    end
    if not port then port = 22003 end

    local player = getPlayerFromName(playerName)
    if not player then
        outputChatBox('Player not found', source, 255, 100, 100)
        return
    end
    redirectPlayer(player, ip, port)
end
addCommandHandler('redirect', redirect, true, false)

function redirectAll(source, _, ip, port)
    if not ip then
        outputChatBox('Syntax: /redirectAll [ip] [port]', source, 255, 100, 100)
        return
    end
    if not port then port = 22003 end
    for k,v in ipairs(getElementsByType"player") do

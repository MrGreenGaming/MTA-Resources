...
local function getPlayerName(player)
  if not isElement or not getElementType(player) == 'player' then
    return false
  end
...
function buildChatWindow(ply)
  ...
  guiSetVisible(chat[ply].wnd, guiGetVisible(wndPlayers))
end
...
function recieveChatMessage(ply, msg)
  ...
  if exports["mrgreen-settings"]:isPlayerIgnoredPM(ply) then
    return
  end

  if getElementData(ply, 'ignored') then
    return
  end
  ...
  if guiGetVisible(chat[ply].memo) == false then
    outputChatBox("#ff0000Press F3 - " .. getPlayerName(ply) .. ": " .. msg,255,255,255,true)
  end
...
addEvent("sb_showPM")
addEventHandler("sb_showPM",root, function()
  togglePmGui()
end)
...
function dxText:create( text, x, y, relative )
	...
	if type(text) == "table" then  --Try an instantiate an existing table of data
		for old,_ in pairs(visibleText) do  -- First, if this one already exists, delete it
			if old.id == text.id then
				old:destroy()
			end
		end
		setmetatable ( text, dxText_mt )
		return true
	end

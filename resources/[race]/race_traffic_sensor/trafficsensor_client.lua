...
addEventHandler('onClientResourceStart', g_ResRoot,
	function()
		-- Bigdar.setHotKey( 'F4' ) -- Uncommented by KaliBwoy, added to settings menu
	end
)
...
function Bigdar.render()
	...
	local dim = getElementData(localPlayer, "dim")
	for i,player in ipairs(Bigdar.allPlayers) do
		if player ~= g_Me and not isPedDead(player) and not isPlayerFinished(player) and (dim == getElementData(player, "dim")) then
...
function showTrafficSensor()
	Bigdar.enabled = true
end

function hideTrafficSensor()
	Bigdar.enabled = false
end
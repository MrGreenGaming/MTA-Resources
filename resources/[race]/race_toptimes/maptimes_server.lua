...
function SMaptimes:makeDatabaseTableName( raceModeName, mapName )
	return (tableprefix[raceModeName] or ('race maptimes ' .. raceModeName)) .. ' ' .. mapName
end
...
function SMaptimes:checkIsSorted(msg)
	...
	for i=2,#self.dbTable.rows do
		...
		if prevTime == time then
			...
			if prevDate > date and msg ~= 'Presort' then
...
function SMaptimes:getToptimes( howMany )
	...
	if (not reversed[self.raceModeName]) then
		for i=1,howMany do
		...
		local j = 1
		for i=#self.dbTable.rows, #self.dbTable.rows - howMany, -1 do
			if i >= 1 then
				result[j] = {
								timeText	= self.dbTable.rows[i].timeText,
								playerName	= self.dbTable.rows[i].playerName
...
--
---------------------------------------------------------------------------
function SMaptimes:getIndexForPlayer( player, bDontReverse )
	...
	if self.statsKey == 'serial' then
		...
		for i,row in ipairs(self.dbTable.rows) do
			if serial == row.playerSerial then
				if bDontReverse or (not reversed[self.raceModeName]) then
					return i
				else
					return #self.dbTable.rows - i + 1
				end
			end
		end
	else
...
--
---------------------------------------------------------------------------
function SMaptimes:getPositionForTime( time, dateRecorded, bDontReverse )
	if not reversed[self.raceModeName] then
		for i,row in ipairs(self.dbTable.rows) do
	...
	elseif bDontReverse then
		for i=#self.dbTable.rows, 1, -1 do
			local row = self.dbTable.rows[i]
			if time > row.timeMs then
				return i + 1
			end
			if time == row.timeMs and dateRecorded < row.dateRecorded then
...
function SMaptimes:getTimeForPlayer( player )
	...
	local i = self:getIndexForPlayer( player, true )
...
function SMaptimes:setTimeForPlayer( player, time, dateRecorded )
	...
	local oldIndex = self:getIndexForPlayer( player, true )

	if not oldIndex then
		-- No entry yet, so add it to the end
		oldIndex = self:addPlayer( player )
		if oldIndex ~= self:getIndexForPlayer( player, true ) then
			outputError( "oldIndex ~= self:getIndexForPlayer( player ) " .. getPlayerName(player) ..' old=' .. oldIndex .. ' i=' ..self:getIndexForPlayer( player, true ) .. ' t=' .. time)
		end
	end
	...
	row.timeMs			= time
	row.timeText		= (not score[self.raceModeName]) and self:timeMsToTimeText(time) or time
	row.dateRecorded	= dateRecorded

	-- Put it back in at the correct position to maintain sort order
	local newIndex = self:getPositionForTime( row.timeMs, row.dateRecorded, true )
...
function SMaptimes:deletetime(place)
	if (not reversed[self.raceModeName]) then
		...
		place = #self.dbTable.rows - (tonumber(place) or 1) + 1
	end

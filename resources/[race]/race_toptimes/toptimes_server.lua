...
addEventHandler('onManhuntRoundEnded', g_Root,
	function(results)
		if g_SToptimesManager then
			for i=#results, 1, -1 do
				g_SToptimesManager:playerFinished( results[i].player, results[i].time)
			end
		end
	end
)

addEventHandler('onPlayerFinishDD', g_Root,
	function(rank)
		if g_SToptimesManager and tonumber(getElementData(source, 'kills')) and tonumber(getElementData(source, 'kills')) > 0 then
			g_SToptimesManager:playerFinished( source, tonumber(getElementData(source, 'kills')))
		end
	end
)

addEventHandler('onPlayerWinDD', g_Root,
	function()
...
addEventHandler('onPlayerFinishShooter', g_Root,
	function(rank)
		if g_SToptimesManager and tonumber(getElementData(source, 'kills')) and tonumber(getElementData(source, 'kills')) > 0 then
			g_SToptimesManager:playerFinished( source, tonumber(getElementData(source, 'kills')))
		end
...
addEventHandler('onPlayerWinShooter', g_Root,
	function()
		-- timer to wait for clientside confirmation of a kill from the last killed guy
		setTimer(function(source)
			if g_SToptimesManager and tonumber(getElementData(source, 'kills')) and tonumber(getElementData(source, 'kills')) > 0 then
...
function getTimeForPlayer( player )
	return g_SToptimesManager.mapTimes:getTimeForPlayer( player )
end

function getIndexForPlayer( player, b )
	return g_SToptimesManager.mapTimes:getIndexForPlayer( player, b )
end

function getPositionForTime (newTime, b)
	return g_SToptimesManager.mapTimes:getPositionForTime( newTime, getRealDateTimeNowString(), b )
end
...
function SToptimesManager:playerFinished( player, newTime, dateRecorded )
	...
	local newPos	= self.mapTimes:getPositionForTime( newTime, dateRecorded, true )

	-- See if time is an improvement for this player
	if not oldTime or (not reversed[self.mapTimes.raceModeName] and newTime < oldTime) or (reversed[self.mapTimes.raceModeName] and newTime > oldTime) then

		local oldPos	= self.mapTimes:getIndexForPlayer( player, true )
		...
		if reversed[self.mapTimes.raceModeName] or newPos <= self.displayTopCount then
			self:updateTopText()
		end
	end

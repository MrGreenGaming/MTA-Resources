...
function GhostPlayback:destroy( finished )
	...
	if self.updateGhostState_HANDLER then removeEventHandler( "onClientPreRender", g_Root, self.updateGhostState_HANDLER ) self.updateGhostState_HANDLER = nil end
...
function GhostPlayback:createNametag()
	self.nametagInfo = {
		name = "Ghost (" .. removeColorCoding(globalInfo.racer) .. ")",
...
function GhostPlayback:startPlayback()
	...
	addEventHandler( "onClientPreRender", g_Root, self.updateGhostState_HANDLER )
end

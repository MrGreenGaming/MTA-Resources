...
function wasted()
	if exports.race:getRaceMode() == "Destruction derby" then
		if hitby[source] and isElement(hitby[source][3]) and getElementHealth(hitby[source][3]) > 0.1 and (getTickCount() - hitby[source][2]) < 10000 then
			...
			exports.messages:outputGameMessage(string.gsub (getElementData(source, 'colorNick') or getPlayerName(source), '#%x%x%x%x%x%x', '' ) .. " was killed by " .. (getElementData(hitby[source][3], 'colorNick') or hitby[source][1]), root, 2.5, 255, 255, 255)
...
function shooterKill(killer)
	if exports.race:getRaceMode() == "Shooter" then
		if exports.race:getShooterMode() == "shooter" then
			local victim = source
			local a = string.gsub (getElementData(victim, 'colorNick') or getPlayerName(victim), '#%x%x%x%x%x%x', '' )
			local b = string.gsub (getElementData(killer, 'colorNick') or getPlayerName(killer), '#%x%x%x%x%x%x', '' )

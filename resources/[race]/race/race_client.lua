...
g_Me = getLocalPlayer()
-- Edit #1, add rustler to weapon list
...
addEventHandler('onClientResourceStart', g_ResRoot,
	function()
		...
		for name,id in pairs(g_ModelForPickupType) do
			...
			engineSetModelLODDistance( id, 180 )
		end
...
function TitleScreen.init()
	...
	-- Edit #2, fit image size
	g_GUI['titleImage'] = guiCreateStaticImage(screenWidth/2-256, screenHeight/2-256+adjustY, 512, 200, 'img/title.png', false)
...
function TravelScreen.init()
    local screenWidth, screenHeight = guiGetScreenSize()
	-- Edit #3, travel image position
	local time = getRealTime()
	if time.monthday == anniversary.day and time.month+1 == anniversary.month then
		g_GUI['travelImage']   = guiCreateStaticImage(screenWidth/2-256, screenHeight/2-20, 512, 256, 'img/travelling_anniversary.png', false, nil)
	else
...
local loadedClientMap = true		-- true at start cause race only sends notifyLoadingMap after a map change, not on race start or client join
local loadingMapResourceName = nil
-- Called from server
...
_addEventHandler('onClientResourceStart', root, function(r)
...
function checkMapLoaded ()
	if not loadedClientMap then
		return setTimer(function() checkMapLoaded() end , 50, 1)
	end
	-- outputDebugString( 'maploaded' )
	...
	function(elem)
		...
		outputDebug( 'CHECKPOINT', 'onClientColShapeHit'
						...
						.. ' g_Vehicle:' .. tostring(g_Vehicle)
							-- Edit #5, warning fix
						.. (isElement(g_Vehicle) and (' isVehicleBlown(g_Vehicle):' .. tostring(isVehicleBlown(g_Vehicle))) or '')
						.. ' g_Me:' .. tostring(g_Me)
...
function handleHitPickup(pickup)
	local nitroLevel, nitroActive
	if pickup.type == 'vehiclechange' then
		nitroLevel, nitroActive = getVehicleNitroLevel(g_Vehicle), isVehicleNitroActivated(g_Vehicle)
		...
		if nitroLevel then
			...
			setVehicleNitroLevel(g_Vehicle, nitroLevel)
			setVehicleNitroActivated(g_Vehicle, nitroActive)
		end
		vehicleChanging(g_MapOptions.classicchangez, pickup.vehicle)
	elseif pickup.type == 'nitro' then
		if getVehicleUpgradeOnSlot(g_Vehicle, 8) == 0 then
			addVehicleUpgrade(g_Vehicle, 1010)
		else
			setVehicleNitroLevel(g_Vehicle, 1)
		end
	elseif pickup.type == 'repair' then
		fixVehicle(g_Vehicle)
	end
	triggerServerEvent('onPlayerPickUpRacePickupInternal', g_Me, pickup.id, pickup.respawn, nitroLevel, nitroActive)
...
function updateTime()
	...
	local msPassed = tick - (g_TimePassedStartTick or g_StartTick)
	if not isPlayerFinished(g_Me) then
		g_dxGUI.timepassed:text(msToTimeStr(msPassed))
	end
	local timeLeft = g_Duration - tick + g_StartTick
	...
	function(dataName)
		...
		if dataName == 'race.spectating' then
			if isPlayerSpectating(source) then
				Spectate.validateTarget( source, true )	-- No spectate at this player
			end
		end
	end
)
...
function checkWater()
    if g_Vehicle then
        if not g_WaterCraftIDs[getElementModel(g_Vehicle)] then
            ...
            elseif g_MapInfo.modename ~= 'Sprint' and g_MapInfo.modename ~= 'Never the same' and isElementInWater(g_Vehicle) and not isPlayerRaceDead(g_Me) and not isPlayerFinished(g_Me) and getElementData(g_Me, "player state") ~= "spectating" and getElementData(g_Me, "player state") ~= "away" and g_MapOptions then
                setElementHealth(g_Me,0)
                triggerServerEvent('onRequestKillPlayer',g_Me,"water")
            end
        end
...
function showVehicleIcon(checkpoint)
    ...
    if not vcEnabled or not checkpoint.vehicle then return end
...
function checkpointReached(elem, matchingDimension)
	...
	--end
	triggerServerEvent('onPlayerReachCheckpointInternal', g_Me, g_CurrentCheckpoint, getVehicleNitroLevel(g_Vehicle), isVehicleNitroActivated(g_Vehicle))
...
function setTimeLeft(timeLeft)
	...
	triggerEvent('onClientMapTimeLeft', g_Me, g_Duration )
end
...
-- Edit #6, added features for spectating from here till end file
Spectate.blockLeaveManual = false
Spectate.blockLeaveManualTimer = nil
Spectate.lockPlayer = false

-- Request to switch on
function Spectate.start(type,block)
	outputDebug( 'SPECTATE', 'Spectate.start '..type )
	assert(type=='manual' or type=='auto', "Spectate.start : type == auto or manual")
	Spectate.blockManual = false
	if block == true then
		Spectate.blockLeaveManual = true
	else
		Spectate.blockLeaveManual = false
	end
	if type == 'manual' then
...
function Spectate.allowUnspectate()
	Spectate.blockLeaveManual = false
...
function Spectate._start()
	...
	if Spectate.lockPlayer and Spectate.isValidTarget(Spectate.lockPlayer) then
		...
		Spectate.setTarget( Spectate.findNewTarget(g_Me,-1) )	-- changed to -1 to fix spectating the last person instead of the first when finished
	end
...
function Spectate.validateTarget(player, ignoreLockplayer)
...
function Spectate.setTarget( player )
	...
	if Spectate.active and Spectate.savePos and not Spectate.blockLeaveManual then
		guiSetText(g_GUI.speclabel, guiGetText(g_GUI.speclabel) .. "\n\nPress 'B' to join")
	end
end
...
function Spectate.blockAsTarget( player, ticks )
	...
	Spectate.validateTarget(player, true)
end
...
function Spectate.tick()
	...
	if not Spectate.target or ( Spectate.getCameraTargetPlayer() and Spectate.getCameraTargetPlayer() ~= Spectate.target ) or (not Spectate.isValidTarget(Spectate.target) and not Spectate.lockPlayer) then
		Spectate.previous(false)
	end
end
...
addEventHandler('onClientPlayerWasted', g_Root,
	function()
		...
		if player == g_Me then
			if #g_Players > 1 and (g_MapOptions.respawn == 'none' or (tonumber(getElementData(player, 'gcshop.respawntime')) or g_MapOptions.respawntime) >= 10000) then
			...
			Spectate.dropCamera( player, 1000, true )
		end
	end
)
...
function alignVehicleWithUp(classicChangeZ)
	...
	if not classicChangeZ then
		if isVehicleWheelOnGround(vehicle, 0) or isVehicleWheelOnGround(vehicle, 1) or isVehicleWheelOnGround(vehicle, 2) or isVehicleWheelOnGround(vehicle, 3) then
			...
			if vehicleId == 581 or vehicleId == 523 or vehicleId == 509 or vehicleId == 481 or vehicleId == 462 or vehicleId == 521 or vehicleId == 463 or vehicleId == 510 or vehicleId == 522 or vehicleId == 461 or vehicleId == 448 or vehicleId == 468 or vehicleId == 586 or vehicleId == 471 then
...
function kill()
	if Spectate.active then
		if Spectate.savePos and not Spectate.blockLeaveManual then
...
function findPlayerByName(playerPart)
	...
	if isElement(pl) and pl ~= g_Me then
		return pl
	else
		for i,v in ipairs (getElementsByType ("player")) do
			if (string.find(string.lower(getPlayerName(v)),string.lower(playerPart))) and v ~= g_Me then
				return v
			end
		end
    end

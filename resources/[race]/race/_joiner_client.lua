...
function triggerLoaded()
	if not isTransferBoxActive() then
		triggerServerEvent('onLoadedAtClient', resourceRoot, g_Me )
        callSavedEventHandlers( 'onClientResourceStart', resourceRoot )
	else	-- wait untill all resources are loaded
		setTimer(triggerLoaded, 2000, 1)
	end
end
addEventHandler('_onClientResourceStart', g_ResRoot, triggerLoaded)
